  ...
  DateComponent.mixin({
    ...
    buildGotoAnchorHtml: function (gotoOptions, attrs, innerHtml) {
      ...
      if (!forceOff && this.opt('navLinks')) {
        return '<span><span class="fc-todayHeader">Today</span><a class="fc-dateHyperlink"' + attrs +
  ...
  */
  ...
  function Toolbar(calendar, toolbarOptions) {
    ...
    function render() {
      ...
      if (sections) {
        if (!el) {
          el = this.el = $("<div class='fc-toolbar " + toolbarOptions.extraClasses + "' />"); //Show legend
        }
        else {
          el.empty();
        }
  ...
  */
  ...
  var DayGridEventRenderer = EventRenderer.extend({
    ...
    fgSegHtml: function (seg, disableResizing) {
      ...
      if (eventDef.title.search("###") > -1) {
        var newTitle = eventDef.title.split("###");
        titleHtml =
          '<span class="fc-custom-title"><span class="fc-custom-startTime">' + (htmlEscape(newTitle[0] || '') || '&nbsp;') + '</span><span class="fc-custom-jobNumber">'
          + (htmlEscape(newTitle[1] || '') || '&nbsp;') + '</span><span class="fc-custom-customerInfo">' + (htmlEscape(newTitle[2] || '') || '&nbsp;') + '</span></span>';   //to add classes for jobNumber and start time by SFB

      } else {
        var customTitle = eventDef.title.split("   ");
        titleHtml =
          '<span class="fc-title"><span class="fc-custom-jobNumber">' + (htmlEscape(customTitle[0] || '') || '&nbsp;') + '</span>' + (htmlEscape(customTitle[1] || '') || '&nbsp;')
          + '</span>';
      }
