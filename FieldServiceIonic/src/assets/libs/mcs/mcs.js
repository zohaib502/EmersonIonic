 ...
 * Oracle Mobile Cloud Service JavaScript SDK for Cordova, Release: 18.1.3.0, E93562-01
 */
...
(function (g) {
  ...
  function Platform(logger, utils) {
    ...
    //this.initGPSLocation = function () {}; //10-19-18 Radheshyam kumar did not found any usages.
  ...
  function Analytics(backend, platform, utils, logger) {
    ...
    this._setEvents = function (events) {
      _events = events;
    };
    ...
    this.flush = function (successCallback, errorCallback) {
      for (var i = 0; i < _events.length; i++) {
        // 01/30/2019 -- Mayur Varshney -- restrict passing lat-long in analytics
        ...
        if (_events[i].name == "context") {
          _events[i].properties.locality = _location.locality ? _location.locality : '';
          _events[i].properties.region = _location.region ? _location.region : '';
          _events[i].properties.postalCode = _location.postalCode ? _location.postalCode : '';
          _events[i].properties.country = _location.country ? _location.country : '';
        }
      }
      /**
       * 11/10/18 Suraj Gorai - set condition if _events have any date then its call "analytics/events" otherwise not
       */
      if (_events.length > 0) {
    ...
    // 01/30/2019 -- Mayur Varshney -- calling function from cloudservice to set custom location in local variable for analytics
 ...
 * });
  ...
  function BasicAuthorization(config, backend, appKey, utils, platform, logger) {
    ...
    this.authenticate = function (username, password, successCallback, errorCallback) {
      ...
      // 02/26/2019 -- Mayur Varshney -- Comment logout function to prevent authentication popup
      // this.logout();
    ...
    this._anonymousTokenResponseConverter = function (response) {
      return {
        orgResponse: response.orgResponse,
        anonymousAccessToken: response.authorizationToken
      };
    };
  ...
  function OAuthAuthorization(config, backend, appKey, utils, platform, logger) {
    ...
    this._anonymousTokenResponseConverter = function (response) {
      return {
        orgResponse: response.orgResponse,
        anonymousAccessToken: response.orgResponse.data.access_token
      };
    };
  ...
  function SSOAuthorization(config, backend, appKey, utils, platform, logger) {
    ...
    this.setAccessToken = function (ssotoken, successCallback) {
      var response = new NetworkResponse(200, ssotoken)
      _this._authenticateSuccess(response, response.data.access_token);
      expiredTime = Date.now() + response.data.expires_in * 1000;
   ...
   */
   ...
   this.setAccsHeaderValue = function(header) {
     accsheader = header;
   }
    ...
    this.invokeCustomCodeJSONRequest = function (path, method, data, successCallback, errorCallback) {
      if (method in httpMethods) {
        ...
        headers[utils.HEADERS.ACCS_TOKEN] = accsheader;
