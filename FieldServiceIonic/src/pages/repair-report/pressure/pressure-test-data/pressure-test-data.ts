...
/**
 * Generated class for the PressureTestDataPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-pressure-test-data',
  templateUrl: 'pressure-test-data.html',
})
export class PressureTestDataPage {

  showLoader: boolean = false;
  toggleAllaccordion: boolean = false;
  expandOuterAccordion: any = {};
  Enums: any;
  fileName: any = 'TestData_Pressure_Tab';
  ...
  TestDataObj: any = {
    TDPF_PK_ID: null,
    PRESSURE_MEASUREMENT_SET_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_CDTP_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_ACTUAL_SET_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_PRE_OPEN_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_RESEAT_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_SEAT_TIGHTNESS_PRESSURE_UOM: null,
    PRESSURE_MEASUREMENT_BACK_PRESSURE_TEST_PRESSURE_UOM: null,
    RING_SETTINGS_UPPER_RING_SETTINGS_UOM: null,
    RING_SETTINGS_LOWER_RING_SETTINGS_UOM: null,
  };
  selectedProcess: any;
  ...
  getLovs() {
    this.localServiceSdr.getLookups(this.accordionData.TestDataPressureLovs).then((res: any) => {
      this.Lookups = [];
  ...
  getExistingData() {
    this.localServiceSdr.getTestDataPressureData(this.TestDataObj.REPORTID).then((res: any): any => {
      if (res && res.length) {
        this.isRecordExists = true;
        this.TestDataObj = JSON.parse(JSON.stringify(res[0]));
  ...
  addNoValueToUOM() {
    for (const key in this.TestDataObj) {
      if (key.indexOf("_UOM") == key.length - 4) {
        const nonUOMKey = key.substring(0, key.indexOf("_UOM"));
        if (this.showValidationClass(this.TestDataObj[nonUOMKey]) && !this.TestDataObj[key]) {
          this.TestDataObj[key] = 'No Value';
        }
      }
    }
  }
  ...
  searchModal(elementName, key) {
    ...
    if (elementName != "TestResultPressure") {
      dataArray.push({ "LookupID": -1, "LookupValue": "Other" });
    }
    if (key.indexOf("_UOM") == key.length - 4) {
      let nonUOMKey = key.substring(0, key.indexOf("_UOM"));
      if (!this.showValidationClass(this.TestDataObj[nonUOMKey])) {
  ...
  toggleAccordion(event) {
    if (this.toggleAllaccordion) {
      for (let key in this.accordionData.AccordionKeys) {
        let val: any = this.accordionData.AccordionKeys[key];
        if (!this.TestDataObj[val + "_NA"] || this.TestDataObj[val + "_NA"] == 'false' || this.TestDataObj[val + "_NA"] == false) {
  ...
  confirmAlert($event, accordionName) {
    ...
    if (isChecked == true) {
      ...
      if (hasValue) {
          ...
          buttons: [
            {
              ...
              handler: () => {
                this.TestDataObj[accordionName + '_' + accordionName + "_NA"] = false;
              }
            },
            {
              ...
              handler: () => {
                ...
                this.TestDataObj[accordionName + '_' + accordionName + "_NA"] = true;
              }
            }
          ]
        });
        ...
        this.TestDataObj[accordionName + '_' + accordionName + "_NA"] = isChecked;
      }
    } else {
      this.TestDataObj[accordionName + '_' + accordionName + "_NA"] = isChecked;
      this.clearAccordion(accordionName);
  ...
  checkIfAccordionHasValue(accordionName) {
    let hasValue = false;
    for (const k in this.TestDataObj) {
      if (k.indexOf(accordionName) == 0) {
        if (this.TestDataObj[k] != '' && this.TestDataObj[k] != null) {
          hasValue = true;
          break;
        }
      }
  ...
  manageOtherValueValidation(key) {
    if (key.indexOf("_OT") == key.length - 3) {
      ...
      if (this.TestDataObj[nonOtherKey] == 'Other' && !this.TestDataObj[key]) {
  ...
  manageUOMValidation(key) {
    if (key.indexOf("_UOM") == key.length - 4) {
      ...
      if (this.showValidationClass(this.TestDataObj[key]) && !this.TestDataObj[nonUOMKey]) {
  ...
  insertData() {
    ...
    for (const a in this.TestDataObj) {
    ...
    this.localServiceSdr.insertOrUpdateData(this.TestDataObj, this.isRecordExists, 'TDPF_PK_ID', 'TESTDATAPRESSUREFLOW').then((res): any => {
    }).catch((error: any) => {
