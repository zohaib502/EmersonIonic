...
export class SolutionPage {
  ...
  addSolutionNetworkValues() {
    if (this.parentReferenceId == '3') {
      this.localServiceSdr.getLookupsByLookupType("AsFoundConditionNetwork").then((item: any) => {
        this.solutionNetworkValues = JSON.parse(JSON.stringify(item));
        this.solutionNetworkValues.unshift({ "LookupID": -2, "LookupValue": "No Value" });
        this.solutionNetworkValues.push({ "LookupID": -1, "LookupValue": "Other" });
      })
    }
  }
    ...
    if (isChecked == 'true' || isChecked == true) {
      switch (accordionName) {
        case 'GearboxMounting':
          this.closeAndClearAccordion('CombinedGearBoxMountingDataAsLeft', accordionName, isChecked);
          break;
        case 'GearboxHousing':
          this.closeAndClearAccordion('CombinedGearBoxHousingDataAsLeft', accordionName, isChecked);
          break;
        case 'GearBoxAccessories':
          this.closeAndClearAccordion('CombinedGearBoxAccessoriesAsLeft', accordionName, isChecked);
          break;
        case 'GearboxSpurGear':
          this.closeAndClearAccordion('CombinedGearBoxSpurGearAsLeft', accordionName, isChecked);
          break;
        case 'NetworkType':
  ...
  selectCalibrationData() {
    this.localServiceSdr.getCalibrationData(this.valueProvider.getCurrentReport().REPORTID).then((res: any) => {
      ...
      for (const i in this.calibrationData) {
        if (
          this.calibrationData[i].CA_ELEMENT == 'N/A' &&
          this.calibrationData[i].CA_OPTION == 'N/A' &&
          this.calibrationData[i].DIRECTION == 'N/A' &&
          this.calibrationData[i].CONDITION == 'N/A'
        ) {
          this.calibrationData[i].RECOMMENDEDACTION = 'N/A';
          this.calibrationData[i].CA_RESULT = 'N/A';
        }
      }
  ...
  setJson() {
    let json = {
      ...
      "GearboxSpurGear": this.asLeftObj.G_SPURGEAR_ALNA,
